    ...
    if not uploaded_files:
        ...
        try:
            ...
            st.session_state["source_files"] = [
                {"name": upload.name, "data": bytes(upload.getvalue())}
                ...
                with st.status("Generating PowerPoint…", expanded=True) as status:
                    try:
                        with tempfile.TemporaryDirectory() as td:
                            ...
                            # Generate the PowerPoint from the original Hudl uploads.
                            # This avoids duplicate normalized columns such as PLAY_TYPE,
                            # which can occur when the cleaned analysis DataFrame is
                            # exported and then normalized a second time.
                            source_files = st.session_state.get("source_files", [])
                            if not source_files:
                                raise ValueError(
                                    "The original uploaded files are no longer available. "
                                    "Reload the opponent files in the sidebar."
                                )

                            report_paths = []
                            for index, source in enumerate(source_files):
                                source_name = Path(source["name"]).name
                                source_path = work / f"{index}_{source_name}"
                                source_path.write_bytes(source["data"])
                                report_paths.append(str(source_path))
