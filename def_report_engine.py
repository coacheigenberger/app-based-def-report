...
def pct(n: float, d: float, whole: bool = True) -> str:
    if d is None or d == 0 or pd.isna(d):
        return "N/A"
    val = 100.0 * n / d
    return f"{val:.0f}%" if whole else f"{val:.1f}%"
...
def situation_detail(df: pd.DataFrame, situation: str) -> Dict[str, Any]:
    ...
    def pct_eff_ypp(col, n=3):
        ...
        rows.sort(key=lambda r: (-float(r[1].strip('%')) if r[1]!="N/A" else 0, r[0]))
        return rows[:n]
    overall=[]
    if not sub.empty:
...
def prediction_rows(df: pd.DataFrame, min_sample=3, max_rows=9) -> List[List[str]]:
    ...
    rows.sort(key=lambda r: (-int(r[1]), -int(r[3].strip('%')), r[0]))
    return rows[:max_rows]
...
def predictability_rows(df: pd.DataFrame, group_cols: List[str], label_join=" / ", min_sample=3, max_rows=10) -> List[List[str]]:
    ...
    rows.sort(key=lambda r: (-int(r[3].strip('%')), -int(r[1]), r[0]))
    return rows[:max_rows]
...
def set_shape_text(shape, text: str):
    ...
    run=p.add_run()
    run.text=str(text)
    # Let the placeholder/theme control the font.
    try:
