...
def prediction_rows(df: pd.DataFrame, min_sample=3, max_rows=9) -> List[List[str]]:
    ...
    for keys, sub in df.groupby(cols):
        ...
        lead_count=max(m["run"], m["pass"])
        top, second, top_n, second_n = top_two_plays(sub)
        play_txt = f"{top} / {second}" if second != "-" else top
        play_pct = f"{pct(top_n,n)} / {pct(second_n,n)}" if second != "-" else pct(top_n,n)
        slice_label=f"{keys[0]} / {keys[1]} / {keys[2]} / {keys[3]}"
        rows.append([slice_label, f"{n} ({reliability_short(n)})", lean, pct(lead_count, n), play_txt, play_pct])
    rows.sort(key=lambda r: (-to_num(str(r[1]).split()[0]), -pct_value(r[3]), r[0]))
    return rows[:max_rows]
...
def predictability_rows(df: pd.DataFrame, group_cols: List[str], label_join=" / ", min_sample=3, max_rows=10) -> List[List[str]]:
    ...
    for keys, sub in df.groupby(group_cols):
        ...
        lead_count=max(m["run"], m["pass"])
        conf=lead_count/len(sub)
        score=predictability_score(conf)
        label=label_join.join(keys) if isinstance(keys, tuple) else str(keys)
        rows.append([label, f"{len(sub)} ({reliability_short(len(sub))})", lean, pct(lead_count, len(sub)), score])
    rows.sort(key=lambda r: (-pct_value(r[3]), -to_num(str(r[1]).split()[0]), r[0]))
    return rows[:max_rows]
...
def reliability_label(n: int) -> str:
    """Sample-size reliability label for coaching context."""
    if n >= 20:
        return "High Sample"
    if n >= 10:
...
def reliability_short(n: int) -> str:
    if n >= 20:
        return "High"
    if n >= 10:
        return "Med"
...
def tendency_score(share: float, n: int, importance: float = 1.0, explosive_rate: float = 0.0, eff_rate: float = 0.0) -> float:
    """
    Rank tendencies by usefulness, not just percentage.
    Higher score favors: strong lean, larger sample, important situation, explosive danger, and efficiency.
    """
    if n <= 0:
        return 0.0
    sample_factor = min(1.0, math.log(n + 1, 20))
    return (share * 100.0) * sample_factor * importance + (explosive_rate * 20.0) + (eff_rate * 10.0)

def efficiency_rate(g: pd.DataFrame) -> float:
    return float(g["EFF_BOOL"].sum()) / len(g) if len(g) else 0.0

def explosive_rate(g: pd.DataFrame) -> float:
    return float(g["IS_EXPLOSIVE"].sum()) / len(g) if len(g) else 0.0

def top_two_plays(sub: pd.DataFrame) -> Tuple[str, str, int, int]:
    if sub.empty:
        return "-", "-", 0, 0
    vc = sub["OFF_PLAY"].value_counts()
    ...
    return str(top), str(second), top_n, second_n

def strongest_alerts(df: pd.DataFrame, min_sample: int = 5, max_rows: int = 6) -> List[List[str]]:
    """Highest-value coach alerts ranked by predictability, sample, importance, efficiency, and explosives."""
    candidates = []
    slice_defs = [
        (["DND"], 1.15),
        (["FIELD_ZONE"], 1.10),
        (["PERSONNEL"], 1.00),
        (["OFF_FORM"], 1.00),
        (["PERSONNEL","OFF_FORM"], 1.20),
        (["DND","OFF_FORM"], 1.25),
        (["PERSONNEL","OFF_FORM","DND"], 1.35),
        (["PERSONNEL","OFF_FORM","FIELD_ZONE","DND"], 1.50),
    ]
    for cols, importance in slice_defs:
        for keys, sub in df.groupby(cols):
            n = len(sub)
            if n < min_sample:
                continue
            ...
            top, second, top_n, second_n = top_two_plays(sub)
            score = tendency_score(share, n, importance, explosive_rate(sub), efficiency_rate(sub))
            note = (
                f"{lean} {pct(lead_count,n)}; "
                f"Top {top} {pct(top_n,n)}; "
                f"EFF {m['eff_pct']}; EXP {m['explosive_pct']}; {reliability_label(n)}"
            )
            candidates.append((score, label, note))
    candidates.sort(key=lambda r: (-r[0], r[1]))
    return [[label, note] for _, label, note in candidates[:max_rows]]

def tell_rows(df: pd.DataFrame, min_sample: int = 5, max_rows: int = 9) -> List[List[str]]:
    """Formation + Personnel + Backfield + Motion tells."""
    rows = []
    cols = ["PERSONNEL","OFF_FORM","BACKFIELD","MOTION"]
    for keys, sub in df.groupby(cols):
        n = len(sub)
        if n < min_sample:
            continue
        ...
        lean = "Run" if m["run"] >= m["pass"] else "Pass"
        top, second, top_n, second_n = top_two_plays(sub)
        label = f"{keys[0]} | {keys[1]} | {keys[2]} | {keys[3]}"
        play_txt = f"{top} / {second}" if second != "-" else top
        play_pct = f"{pct(top_n,n)} / {pct(second_n,n)}" if second != "-" else pct(top_n,n)
        score = tendency_score(share, n, 1.25, explosive_rate(sub), efficiency_rate(sub))
        rows.append((score, [label, f"{n} ({reliability_short(n)})", lean, pct(lead_count,n), play_txt, play_pct]))
    rows.sort(key=lambda r: (-r[0], r[1][0]))
    return [r for _, r in rows[:max_rows]]

def call_sheet_rows(df: pd.DataFrame, min_sample: int = 3, max_rows: int = 9) -> List[List[str]]:
    """Condensed athlete/game-day situation reference."""
    rows = []
    labels = []
    for d in DND_ORDER:
        if len(df[df["DND"] == d]):
            labels.append(("D&D: " + d, df[df["DND"] == d], 1.10))
    for z in ["High Red Zone", "Low Red Zone", "Goal Line", "3 Down Territory", "4 Down Territory"]:
        if len(df[df["FIELD_ZONE"] == z]):
            labels.append(("Zone: " + z, df[df["FIELD_ZONE"] == z], 1.15 if "Red" in z or z=="Goal Line" else 1.00))
    # Include best supported formation/personnel slices
    for col, prefix in [("OFF_FORM", "Form"), ("PERSONNEL", "Pers")]:
        for key, sub in df.groupby(col):
            if len(sub) >= min_sample:
                labels.append((f"{prefix}: {key}", sub, 1.00))
    seen = set()
    candidates = []
    for label, sub, importance in labels:
        if label in seen or len(sub) < min_sample:
            continue
        seen.add(label)
        n = len(sub)
        m = core_metrics(sub)
        ...
        share = lead_count / n
        top, second, top_n, second_n = top_two_plays(sub)
        alert = predictability_score(share)
        if m["explosive"] > 0:
            alert += f"; {m['explosive']} EXP"
        score = tendency_score(share, n, importance, explosive_rate(sub), efficiency_rate(sub))
        candidates.append((score, [label, f"{n} ({reliability_short(n)})", f"{lean} {pct(lead_count,n)}", f"{top} {pct(top_n,n)}", m["eff_pct"], alert]))
    candidates.sort(key=lambda r: (-r[0], r[1][0]))
    return [r for _, r in candidates[:max_rows]]

def data_quality_rows(raw: pd.DataFrame, df: pd.DataFrame, issues: Optional[List[str]] = None, max_rows: int = 9) -> List[List[str]]:
    issues = issues or []
    rows = []
    rows.append(["Raw rows", str(len(raw)), "Imported", "All uploaded file rows before filters.", "", ""])
    rows.append(["ODK=O run/pass", str(len(df)), "Used", "Valid offensive run/pass denominator.", "", ""])
    unknown_play_type = 0
    if "PLAY_TYPE" in raw.columns:
        unknown_play_type = int((raw["PLAY_TYPE"].apply(norm_play_type) == "Unknown").sum())
    rows.append(["Unknown play type", str(unknown_play_type), "Review" if unknown_play_type else "OK", "Rows not classified as run/pass.", "", ""])
    blank_forms = int((df["OFF_FORM"] == "-").sum()) if "OFF_FORM" in df.columns else 0
    rows.append(["Blank formation", str(blank_forms), "Review" if blank_forms else "OK", "ODK=O plays with no formation tag.", "", ""])
    non_numeric_yards = 0
    if "GNLS" in raw.columns:
        non_numeric_yards = int(raw["GNLS"].apply(to_num).isna().sum())
    rows.append(["Nonnumeric yardage", str(non_numeric_yards), "Review" if non_numeric_yards else "OK", "Rows with yardage not read as a number.", "", ""])
    duplicate_rows = int(raw.duplicated().sum())
    rows.append(["Exact duplicate rows", str(duplicate_rows), "Review" if duplicate_rows else "OK", "Full-row duplicates in uploaded file(s).", "", ""])
    for issue in issues:
        rows.append(["Validation note", "-", "Review", issue, "", ""])
    return rows[:max_rows]
...
def defensive_alerts(df: pd.DataFrame) -> List[List[str]]:
    alerts = strongest_alerts(df, min_sample=5, max_rows=6)
    if alerts:
        return alerts[:3]
    # Fallback when sample is too small
    alerts=[]
    if len(df):
...
def retitle_slide(slide, title: str, subtitle: Optional[str] = None):
    """Set the first and optional second text shapes while preserving template formatting."""
    text_shapes = [s for s in slide.shapes if getattr(s, "has_text_frame", False) and s.text_frame]
    if text_shapes:
        set_shape_text(text_shapes[0], title)
    if subtitle and len(text_shapes) > 1:
        set_shape_text(text_shapes[1], subtitle)
...
def populate_report(df: pd.DataFrame, template: str, output: str, opponent: str = "Opponent", games_label: str = "ODK = O only", raw: Optional[pd.DataFrame] = None, issues: Optional[List[str]] = None, min_sample: int = 5):
    ...
    fill_table(tables[0], prediction_rows(df, min_sample=min_sample, max_rows=9))
    ...
    if len(tables)>=3:
        fill_table(tables[0], predictability_rows(df, ["DND"], min_sample=min_sample, max_rows=8))
        fill_table(tables[1], predictability_rows(df, ["OFF_FORM"], min_sample=min_sample, max_rows=6))
        fill_table(tables[2], predictability_rows(df, ["DND","OFF_FORM"], min_sample=min_sample, max_rows=10))
    ...
    # Added v2.2: athlete/staff-ready summary slides
    # Top Defensive Alerts (2-column plan-style slide)
    alert_slide = duplicate_slide(prs, s_plan)
    retitle_slide(alert_slide, "TOP DEFENSIVE ALERTS", "Highest-value tendencies ranked by predictability, sample size, efficiency, and explosives.")
    tables = iter_tables(alert_slide)
    if tables:
        fill_table(tables[0], strongest_alerts(df, min_sample=min_sample, max_rows=3))

    # Predictive Tells (6-column prediction-style slide)
    tells_slide = duplicate_slide(prs, s_pred)
    retitle_slide(tells_slide, "PREDICTIVE TELLS", "Personnel + Formation + Backfield + Motion combinations")
    tables = iter_tables(tells_slide)
    if tables:
        fill_table(tables[0], tell_rows(df, min_sample=min_sample, max_rows=9))

    # Game Day Call Sheet (6-column prediction-style slide)
    call_slide = duplicate_slide(prs, s_pred)
    retitle_slide(call_slide, "GAME DAY CALL SHEET", "Condensed athlete-facing tendency reference")
    tables = iter_tables(call_slide)
    if tables:
        fill_table(tables[0], call_sheet_rows(df, min_sample=max(3, min_sample), max_rows=9))

    # Data Quality / Staff Review (6-column prediction-style slide)
    dq_slide = duplicate_slide(prs, s_pred)
    retitle_slide(dq_slide, "DATA QUALITY CHECK", "Staff review before sharing the deck")
    tables = iter_tables(dq_slide)
    if tables:
        fill_table(tables[0], data_quality_rows(raw if raw is not None else df, df, issues=issues, max_rows=9))

    prs.save(output)

def generate(excel_paths: List[str], template: str, output: str, opponent: str, odk: str="O", min_sample: int = 5) -> Dict[str, Any]:
    ...
    populate_report(df, template, output, opponent=opponent, games_label=games_label, raw=raw, issues=issues, min_sample=min_sample)
    m=core_metrics(df)
...
def main():
    ...
    parser.add_argument("--min-sample", type=int, default=5, help="Minimum snaps for displayed predictive tendencies")
    parser.add_argument("--excel", nargs="+", required=True, help="One or more Excel/CSV files")
    args=parser.parse_args()
    result=generate(args.excel, args.template, args.output, args.opponent, args.odk, min_sample=args.min_sample)
